...
public class Magpie
{
  ...
  public String getResponse(String statement)
  {
    ...
    {
      response = "I bet he is an awesome dude.";
    }
    ...
    {
      response = "Tell me more about this sport.";
    }
    ...
    else if (findKeyword(statement, "I want to", 0) >= 0)  // looks for a "I want to" statement. 
    {
      response = transformIWantToStatement(statement);  // if found, call the method
    }
    else if (findKeyword(statement, "I want", 0) >= 0)  // looks for an "I want" statement
    {
      response = transformIWantStatement(statement);  // if found, call the  method
    }
    ...
    {
      response = transformIsStatement(statement);  // if found, call the method
    }
    else if (findKeyword(statement, "are", 0) >= 0 && (findKeyword(statement, "you", 0) < 0))  // looks for statement that has "are", but not "you"
    {
      response = transformAreStatement(statement);  // if there is an "are", but no "you", call the  method
    }
    ...
    else if (findKeyword(statement, "You are", 0) >= 0)  // looks for a "you are"
    {
      response = transformYouAreToQuestion(statement);
    }
    else if (findKeyword(statement, "I have", 0) >= 0)  // looks for a statement with "I have" 
    {
      response = transformIHaveToQuestion(statement);  // if found, call the  method
    }
    else if (findKeyword(statement, "You have", 0) >= 0)  // looks for a statement with  in it
    {
      response = transformYouHaveToQuestion(statement);  // if found, call the  method
    }
    else if (findKeyword(statement, "has", 0) >= 0)  // looks for a statement with "has" 
    {
      response = transformHasToHaveQuestion(statement);  // if found, call the method
    }
